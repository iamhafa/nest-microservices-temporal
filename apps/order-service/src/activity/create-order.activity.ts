...
export class CreateOrderActivity implements ICreateOrderActivity {
  ...
  async execute(
    createOrderDto: ICreateOrderDto,
    ...
    const totalAmount: number = items.reduce(
      (sum: number, item: IOrderItem) => sum + productPrices[item.product_id] * item.quantity,
      0,
    );
